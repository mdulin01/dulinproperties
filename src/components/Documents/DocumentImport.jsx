...
import { FFB_STATEMENTS, ffbStatementToEntries } from '../../data/ffbStatements';
...
export default function DocumentImport({
  ...
  const existingFingerprints = useMemo(() => {
    ...
    for (const e of expenses || []) {
      ...
      if (e.fingerprint) set.add(e.fingerprint); // stored at import time (uses original description)
      ...
      if (parserKind === 'owner-packet') {
        ...
        mapped = result.transactions.map((tx, idx) => {
          ...
          const entry = {
            ...
            flowType: isIncome ? 'income' : 'expense',
            // Preserve the parser's income classification: SIGONFILE owner
            // distributions and bank interest must NOT be recorded as rent.
            incomeCategory: isIncome
              ? (['owner-distribution', 'interest', 'refund', 'other'].includes(tx.category) ? tx.category : 'rent')
              : undefined,
            imported: false,
          };
          const fp = fingerprintEntry(entry);
  ...
  return (
    <div>
      ...
      {/* FFB Bank presets — verified statement data (src/data/ffbStatements.js),
          keyed from the actual PDFs and balanced to each statement's totals.
          For months not listed just drop the PDF. */}
      {activeSource === 'ffb-bank' && !ffbMonth && entries.length === 0 && (
        <div className="mb-4">
          <p className="text-[11px] text-white/40 mb-2">Quick-load a verified month from the FFB statements (rental account •••5710 only — or drop a PDF below for any other month):</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {FFB_STATEMENTS.map(m => (
              <button
                key={m.id}
                onClick={() => { setFfbMonth(m.id); setEntries(ffbStatementToEntries(m)); }}
              ...
              >
                ...
                <p className="text-[10px] text-white/30 mt-1">{m.periodLabel}</p>
              </button>
            ))}
          </div>
        </div>
